    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        if n_iter % tb_interval == 0 or task == "val":
            ...
            uv_b = findInterestPoints(img_warp, blockSize=2, ksize=1, k=0.1)
            ...
            if len(matches_a_descriptors) >5:
                for index_a in range(5):
...
if __name__ == "__main__":
    ...
    train_agent = Train_model_heatmap_mvsec(config, device=device)
