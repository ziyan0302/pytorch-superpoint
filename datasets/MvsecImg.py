...
class MvsecImg(data.Dataset):
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        ...
        for datasetName, data_info in self.dataset_start_dict.items():
            # take care of starting time for each dataset to avoid static images
            ...
            with h5py.File(file_path, 'r') as hf:
                ...
                # the unit of start_time = sec, transform to frame number  
                start_frame = int(data_info['start_time'] * (1/0.02189))
                # collect information of each dataset: starting frame and actual length
                # after cropping the dataset
                self.dataset_collection[datasetName] = {'start_frame' : start_frame, 'len' : (len(image_raw_ts) - start_frame)}
                # total length of the datasets 
                # determine self.len for dataloader 
                self.len += self.dataset_collection[datasetName]['len']
            
            # cache events to speed up the dataloader 
            ...
            if not os.path.exists(img2events_file):
                with h5py.File(file_path, 'r') as hf:
                    ...
                    # get the events' time frame number appearing between the corresponding camera time frames
    ...
    def __getitem__(self, index):
        ...
        dataset_idx = -1
        # determine which dataset cache would be read 
        ...
        # remove the dataset offset from index
        ...
        with h5py.File(Path(self.dataset_root,file_path), 'r') as hf:
            ...
            # event numbers between every subsequent camera time frame
            self.img2events = np.load(img2events_file)
            ...
            if (1): # ziyan's modification
                ...
                # add intended start offset to avoid static views
                self.raw_image = hf['davis']['left']['image_raw'][index+start_frame]
                # make the sparse events into event volume [18, H, W]
